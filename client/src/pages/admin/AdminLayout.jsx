    ...
    <div className={`w-64 bg-white border-r border-cream-dark/20 flex flex-col ${className}`}>
      ...
      <nav className="flex-1 p-4 space-y-1">
        {links.map(l => {
          ...
          return <Link key={l.path} to={l.path} onClick={() => setSidebarOpen(false)} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${active ? 'bg-baby-pink/20 text-brand-dark shadow-sm' : 'text-brand-light hover:bg-cream hover:text-brand-dark'}`}>{l.icon}{l.label}</Link>;
        })}
      </nav>

      ...
      <div className="card card-hover h-full flex flex-col">
        <div className="relative overflow-hidden aspect-square bg-gradient-to-br from-cream to-baby-pink/20">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {product.featured && <span className="badge bg-brand-warm/20 text-brand-warm shadow-sm">⭐ Featured</span>}
            {disc > 0 && <span className="badge bg-red-50 text-red-500 shadow-sm">-{disc}%</span>}
          </div>
        </div>
        <div className="p-5 flex flex-col flex-1">
          ...
          <div className="mt-auto flex flex-col">
            <div className="flex items-center justify-between mb-3 border-t border-cream-dark/30 pt-3">
              <div className="flex items-baseline gap-2">
                <span className="text-lg font-bold text-brand-dark">${product.price}</span>
                {product.comparePrice > 0 && <span className="text-xs text-brand-muted line-through">${product.comparePrice}</span>}
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={handleAdd} className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-brand-warm text-white rounded-xl text-sm font-semibold hover:bg-brand-dark transition-all shadow-sm active:scale-95">
                <HiOutlineShoppingCart className="w-4 h-4" /> Add to Cart
              </button>
              <Link to={`/product/${product._id}`} className="px-3 flex items-center justify-center rounded-xl bg-brand-warm/10 text-brand-warm hover:bg-brand-warm/20 transition-all shadow-sm tooltip" title="View details">
